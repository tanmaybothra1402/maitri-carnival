-- 202608010025_crm_filter_failopen_and_assignedto.sql
--
-- Two fixes to crm_list_customers (guardrail D1: an empty result must show a
-- reason, never a blank screen staff read as "the data is gone").
--
-- FIX 1 — filters fail OPEN on an unrecognised value, instead of emptying the list
-- (or, for the booleans, throwing). Audited every param:
--   buyerType / status / tier / assigned  -> unknown value returned 0 rows (fail closed)
--   withOrders / callbacksDue             -> non-boolean threw 22P02 (fail hard)
--   sort                                  -> already fell back to default (safe, unchanged)
--   search                                -> free text; "no match" is a correct empty (unchanged)
--   exhibitionId / limit                  -> typed uuid/int casts; client-controlled, left as-is
--                                            (a bad exhibition id SHOULD surface, not be masked)
-- The four enums are normalised to NULL when unrecognised, so the existing
-- `param is null -> no filter` branches now handle a typo as "show all". The two
-- booleans use a liberal truthy test that never throws. Failing open is correct
-- here: the real failure we hit is staff seeing a blank CRM with no explanation,
-- which is worse than a filter quietly not applying.
--
-- FIX 2 — new `assignedTo` param filters to one salesperson's customers in SQL,
-- before the LIMIT. The client's "assigned to me" toggle moves onto it; it used to
-- filter the fetched 500-row page client-side, so a salesperson whose customers
-- sort past row 500 saw an incomplete list of their OWN buyers.
--
-- Whole function restated (house style). The 022 tier-first ORDER BY is preserved
-- verbatim in both the filtered CTE and the jsonb_agg.
