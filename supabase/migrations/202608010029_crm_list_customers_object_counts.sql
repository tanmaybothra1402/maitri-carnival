-- 202608010029_crm_list_customers_object_counts.sql
--
-- crm_list_customers returns an OBJECT { customers, assigneeCounts, unassignedCount,
-- totalCount } for the CRM salesperson-picker counts. Applied AFTER admin-api v44
-- (the shape-tolerant Edge) went live, so there was no breakage window. This is the
-- object shape 026 introduced and 027 reverted; re-applied here now the Edge reads it.
-- Counts computed on the list's exact scope (active + current exhibition). Verified
-- live: assigned sum 145 across the salesperson breakdown; totalCount 875.
