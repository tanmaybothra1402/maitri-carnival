-- 202608010026_crm_list_assignee_counts.sql
--
-- crm_list_customers now returns per-salesperson assignment COUNTS alongside the
-- customer page, in the SAME call, so the CRM salesperson picker can show "UMA (9)"
-- and have that number equal the rows a UMA selection returns.
--
-- RETURN SHAPE CHANGE (still jsonb, so create-or-replace is fine — no drop):
--   was: jsonb ARRAY  of customers
--   now: jsonb OBJECT { customers, assigneeCounts, unassignedCount, totalCount }
-- The only caller is admin-api crmListCustomers, updated in the same ship to unwrap it.
--
-- COUNT SCOPE (the trust-critical decision): the counts are computed from the
-- `scoped` CTE — active customers in the current exhibition — which is EXACTLY the
-- list's scope, BEFORE any content filter and BEFORE the assignee filter. So they
-- are ABSOLUTE assignment totals on the list scope, matching the ground truth
-- (UMA 9 … 40 assigned, 835 unassigned, 875 in scope). They do NOT respect the
-- other active filters (buyer type / status / tier / with-orders): the picker
-- answers "how many has he assigned to each salesperson, in total", a stable roster
-- overview, and a "(0)" means genuinely idle — not "none match the current filter".
-- Trade-off: with another content filter also active, selecting a person shows the
-- intersection (<= the shown count); that is the intended "UMA has 9; N match what
-- you're looking at" reading.
--
-- Whole function restated (house style). The 022 tier-first ORDER BY is preserved
-- verbatim, and every 025 fail-open param + the assignedTo param are unchanged.
